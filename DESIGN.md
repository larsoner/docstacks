...
- Aliases are **remembered but never rewritten**, which is mike's behavior. An alias that already points at the version being deployed — transitively, so pandas's `stable -> 2.1 -> 2.1.3` counts — is treated as one of that deploy's aliases for every manifest purpose, so a bugfix redeploy of the current stable release keeps its `/stable/` URL and its `preferred` flag without the CI job having to work out for itself that the version it is redeploying is the stable one. An inherited alias supplies the URL but never a label: the generated `"<version> (<alias>)"` name is written only for an alias that was actually asked for, so a `retitle`d entry is not silently relabeled by the next routine redeploy. Only the aliases explicitly asked for are created on disk; existing ones are left exactly as they are, chain and all, and an alias pointing at some *other* version is neither touched nor adopted. Moving an alias between versions, and demoting whoever held it, is still `promote`'s concern, because it is the command that reasons about the site as a whole.
...
- `--ignore VERSION` exempts a version from the deployed-world checks. This is an adoption requirement rather than a convenience: the first live run against mne.tools reported seven findings on a site working exactly as intended — six archives built before the theme had a switcher, plus the deliberate `0.20` catch-all pointing into `dev/old_versions/`. A validator that can never reach exit 0 on a healthy site is one nobody puts in cron, so the permanent findings have to be silenceable or the whole feature is decorative. Ignored entries are skipped *before* the request rather than fetched and suppressed, so a nightly job does not hammer dead archives. It covers both directions of the `--site-dir` cross-check, and it deliberately does **not** affect `Manifest.validate()`: a duplicate version or an empty URL is a defect in the file itself, not a fact about the deployed world. An `--ignore` matching nothing is accepted in silence — erroring on it would be the more "correct" behavior and the wrong one, because it means the cron breaks on the day an old entry is finally pruned.
