...
The CI step still passes `--alias stable` when `readlink stable` in the site checkout equals the directory being deployed, but that guard is now unnecessary: `deploy` keeps aliases that already point at the version being deployed, so a bugfix redeploy of the current stable release no longer rewrites the stable entry to `/1.13/`.
Release day is now `cp -a 1.13 /tmp/1.13-html && docstacks promote /tmp/1.13-html 1.13 --base-url https://mne.tools/ --push` followed by `docstacks retitle dev "1.14 (dev)" --push` in a clean clone of the site repo; the temp copy exists because `promote` deletes the target directory before copying the HTML in.
