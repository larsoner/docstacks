...
The squash commit takes the tree of the window's parent, so a tree shrink (deleting old versions or zips) only leaves the repo once it sits below the window: MNE's September 2026 reroot deleted first, added one more commit, then ran `prune --keep 1 --base 6836a9508` (the 2013 initial commit), which measured a 1.04 GiB push and an 863 MiB repo, whereas the same reroot with `--keep 20` measured 2.4 GiB and would have been rejected.
